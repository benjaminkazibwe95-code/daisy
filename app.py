...
# ============================================================
# LIVE WEB SEARCH — real, current results from the open web.
# Daisy's dictionary/law engine and Claude's own training data
# both go stale (prices, scores, news, "who is the current...",
# anything released after the fact). When the person turns the
# search toggle on, this fetches live results from DuckDuckGo's
# HTML endpoint — no API key required, keeping this free to run
# on Render's free tier the same way the rest of Daisy is.
# ============================================================
...
def web_search(query, max_results=5, timeout=6):
    """
    Search the live web for `query` and return a list of
    {title, snippet, url} dicts (possibly empty). Never raises —
    any network/parsing failure just means an empty result list,
    so a flaky connection degrades Daisy back to her own
    knowledge instead of breaking the request.
    """
    if not SEARCH_ENABLED or not _SEARCH_AVAILABLE or not query or not query.strip():
        return []
    try:
        resp = _requests.post(
        ...
        print(f"[SEARCH] web_search failed for {query!r}: {e}")
        return []
...
- LIVE WEB SEARCH: sometimes the context you're given includes a block of live search results Daisy just pulled from the internet for this exact question — real, current pages, not your own training data. When that block is present, actually use it: it's more current than anything you already know, especially for prices, scores, news, releases, or "who is the current..." questions. Weave the information in naturally, the way you'd explain something you just looked up for a friend — don't dump the raw snippets. At the very end of your reply, on its own short line, add a **Sources:** line with the 2-3 most relevant links as plain markdown (e.g. `[Site name](https://example.com)`), only for links you were actually given, never invented. If no search block is present, answer from what you already know as usual — don't claim you searched when you didn't.
...
def speak_naturally(question, raw_fact, custom_instructions=None, image_data=None, remembered_facts=None, search_results=None):
    """
        ...
        if search_results:
            ...
            for r in search_results[:5]:
                ...
                if not title or not url:
                    continue
                entries.append(f"- {title} — {snippet} ({url})")
            if entries:
                search_block = (
                    "[LIVE WEB SEARCH RESULTS, fetched just now for this question — "
                    "not visible to the user as-is, use per the LIVE WEB SEARCH rule:\n"
                    + "\n".join(entries) + "]\n\n"
                )
...
def serve_asset_links():
    """
    Proves to Google Play / Chrome that ug.co.daisy.app (the Android TWA)
    is authorized to open this domain as a trusted web activity, so the
    installed app opens full-screen with no browser URL bar.
    """
...
def ask():
    ...
    want_search = bool(data.get("web_search"))  # person tapped the "search the internet" toggle
    ...
    # Skipped when the person explicitly asked Daisy to search live —
    # a cached answer could predate whatever just changed on the web.
    cached = get_correction(question) if not want_search else None
    if cached:
        return jsonify({"answer": cached, "source": "learned"})

    result = ask_daisy(question, learned, history)

    # LIVE WEB SEARCH — only runs when the person turned the toggle on.
    # Failures are silent (empty list), so a flaky connection just
    # falls back to Daisy's own knowledge instead of breaking the reply.
    search_results = web_search(question) if want_search else []
    ...
    final_answer, learned_fact, memory_fact = speak_naturally(
        question, raw_answer, custom_instructions,
        remembered_facts=remembered_facts, search_results=search_results,
    )
    result["answer"] = final_answer
    result["raw_fact"] = raw_answer
    result["memory_fact"] = memory_fact
    result["used_web_search"] = bool(search_results)
    if learned_fact and not want_search:
